...
void	Response::response(int client_fd, ServerConfig &serverConfig, RequestInfo &requestInfo)
{
	...
	std::cout << "request: " << requestInfo.path << "   " << requestInfo.fullPath << "   " << serverConfig.routes.begin()->second.path << "\n";
	...
	if (routeIt != serverConfig.routes.end()) {
		...
		std::string file = std::string(route.root_directory) + "/" + std::string(route.default_file);
		setBody(file);
		std::cout << "file: " << file << "   \n" ;
	} else {
