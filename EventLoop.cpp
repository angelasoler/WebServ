...
int		EventLoop::handleClient(int client_fd, int poll_index)
{
	...
	else if (bytes_read == 0)
	{
		...
		close(client_fd);
		refServer.poll_fds.erase(refServer.poll_fds.begin()+poll_index);
		return (1);
	}
	else
	{
...
int		EventLoop::treatHTTPMethod(std::string text)
{
